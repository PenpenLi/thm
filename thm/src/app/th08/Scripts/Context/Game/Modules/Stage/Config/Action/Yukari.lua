return {
    [StageDefine.ActionType.PLAYER_STAND] = function (sprite,prevAction)
        ...
        if prevAction == StageDefine.ActionType.PLAYER_MOVE_LEFT or prevAction == StageDefine.ActionType.PLAYER_MOVE_RIGHT then
            table.insert( actions,cc.Animate:create(AnimationCache.getSheetRes("yukari_move_left",1/30)):reverse())
        ...
        table.insert( actions,cc.CallFunc:create(function() 
            sprite:playAnimationForever(AnimationCache.getSheetRes("yukari_stand_normal"))
        end))
    ...
    [StageDefine.ActionType.PLAYER_MOVE_LEFT] = function (sprite,prevAction)
        ...
        sprite:runAction(cc.Sequence:create({
            cc.Animate:create(AnimationCache.getSheetRes("yukari_move_left_start")),
            cc.CallFunc:create(function() 
                -- sprite:stopAllActions()
                sprite:runAction(cc.RepeatForever:create(
                    cc.Animate:create(AnimationCache.getSheetRes("yukari_move_left_sustain"))
                ))
            end)
        }))
    end,
